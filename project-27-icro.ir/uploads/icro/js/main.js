...
function resizeFunc() {
    if ($(document).width() < 992) {
        $(document).ready(function () {
            $('.header-hamburger-btn a').on('click', function () {
               
                console.log($(this).hasClass('open-now'))
                if($(this).hasClass('open-now') == true){
                    $(".menu__desktop").css('transform', 'translate3d(0%,-100%,0)');
                    $('body').removeClass('mm-wrapper_opened');
                    console.log('hi')
                }
                else{
                    $('body').addClass('mm-wrapper_opened');
                    $(".menu__desktop").css('transform', 'translate3d(0%,0%,0)');
                }
        ...
        (function (e) {
            var mymenu = new Mmenu(document.querySelector("#espritmenu"), { extensions: ["theme-dark", "position-front", "position-top", "pagedim-black", "fx-panels-slide-100", "fx-listitems-slide"], navbars: [, !0], navbar: { add: !0, title: "منوی اصلی" } })
        }).apply(this, [jQuery]);
        $('.sm').removeClass('sm');
        ...
        $('#espritmenu > ul').addClass('sm sm-simple sm-rtl');
