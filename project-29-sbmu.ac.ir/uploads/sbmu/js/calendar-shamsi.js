...
!(function () {
  ...
  function s(b) {
    ...
    return (
      (c = Math.floor(b - 0.5) + 0.5),
      (d = c - a),
      (e = Math.floor(d / 146097)),
      (f = n(d, 146097)),
      (g = Math.floor(f / 36524)),
      (h = n(f, 36524)),
      (i = Math.floor(h / 1461)),
      (j = n(h, 1461)),
      (k = Math.floor(j / 365)),
      ...
      4 != g && 4 != k && m++,
      (o = c - r(m, 1, 1)),
      (q = c < r(m, 3, 1) ? 0 : p(m) ? 1 : 2),
      (month = Math.floor((12 * (o + q) + 373) / 367)),
      (day = c - r(m, month, 1) + 1),
      new Array(m, month, day)
    );
  }
  ...
  function t(a, c, d) {
    ...
    return (
      (e = a - (a >= 0 ? 474 : 473)),
      (f = 474 + n(e, 2820)),
      d + (c <= 7 ? 31 * (c - 1) : 30 * (c - 1) + 6) + Math.floor((682 * f - 110) / 2816) + 365 * (f - 1) + 1029983 * Math.floor(e / 2820) + (b - 1)
    );
  }
  ...
  function u(a) {
    ...
    return (
      (a = Math.floor(a) + 0.5),
      (e = a - t(475, 1, 1)),
      (f = Math.floor(e / 1029983)),
      (g = n(e, 1029983)),
      1029982 === g ? (h = 2820) : ((i = Math.floor(g / 366)), (j = n(g, 366)), (h = Math.floor((2134 * i + 2816 * j + 2815) / 1028522) + i + 1)),
      (b = h + 2820 * f + 474),
      b <= 0 && (b -= 1),
      (k = a - t(b, 1, 1) + 1),
      (c = k <= 186 ? Math.ceil(k / 31) : Math.ceil((k - 6) / 30)),
      (d = a - t(b, c, 1) + 1),
      new Array(b, c, d)
    );
  }
  ...
  function w(a, b, c) {
    var d = r(a, b + 1, c) + Math.floor(0.5) / 86400,
      e = u(d),
      f = o(d);
  ...
  function x(a) {
    ...
    return (
      ...
      6 == b[3]
        ...
        : 1 == b[3]
        ? (c.weekDayNumber = 3)
        : 0 == b[3] && (c.weekDayNumber = 2),
      (c.year = b[0]),
      (c.month = b[1]),
      (c.day = c.weekDayNumber),
      (c.date = b[2]),
      (c.hours = a.getHours()),
      (c.minutes = a.getMinutes() < 10 ? "0" + a.getMinutes() : a.getMinutes()),
      (c.seconds = a.getSeconds()),
      (c.milliseconds = a.getMilliseconds()),
      (c.timeZoneOffset = a.getTimezoneOffset()),
      c
    );
  }
  ...
  (A.prototype = {
    ...
    utc: function (a) {
      if (a) return new persian_Date(a).utc();
    ...
    unix: function (a) {
      if (a) return new persian_Date(1e3 * a);
    ...
    (persian_Date = A),
    (pDate = A),
    (persian_Date.unix = persian_Date.prototype.unix),
    (persian_Date.utc = persian_Date.prototype.utc);
})(),
  persian_Date().format();
var calendar = {
  init: function (a) {
  ...
  startCalendar: function () {
    ...
    function n(a, b) {
      for (date = persian_Date([b, a, 1]), days = 0; date.pDate.month === a; ) days++, (date = persian_Date([b, a, days]));
      return days - 1;
    }
    ...
    function s(a, b) {
      $($("tbody.event-calendar tr")).each(function (a) {
        ...
        (workDate = persian_Date([b, a, 1])),
        (offset = workDate.gDate.getDay() + 1),
        7 == offset && (offset = 0);
      for (var c = 0; c < offset; c++) $("tbody.event-calendar tr.1").append("<td></td>");
      for (var d = 1; d <= n(a, b); d++)
        (index = d + offset),
          index < 8
            ? $("tbody.event-calendar tr.1").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>")
            : index < 15
            ? $("tbody.event-calendar tr.2").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>")
            : index < 22
            ? $("tbody.event-calendar tr.3").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>")
            : index < 29
            ? $("tbody.event-calendar tr.4").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>")
            : index < 36
            ? $("tbody.event-calendar tr.5").append('<td date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>")
            : $("tbody.event-calendar tr.6").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>");
    ...
    function t() {
      $(".day-event").each(function (a) {
        ...
        var jD = JalaliDate.jalaliToGregorian(d, b, c);
        var dateToSend = jD[0] + "/" + jD[1] + "/" + jD[2];
        window.location.href = window.location.href.substring(0, window.location.href.lastIndexOf("?")) + "?date=" + dateToSend;
      });
    }
    ...
    function u() {
      $("tbody.event-calendar td").on("click", function (a) {
        ...
        var jD = JalaliDate.jalaliToGregorian(y, b, c);
        var dateToSend = jD[0] + "/" + jD[1] + "/" + jD[2];
        window.location.href = window.location.href.substring(0, window.location.href.lastIndexOf("?")) + "?date=" + dateToSend;
      });
    }
    ...
    function v() {
      $(".remove").click(function () {
        if (this.checked) {
          ...
          $('.day[date-month="' + a + '"][date-day="' + b + '"][data-number="' + c + '"]').slideUp("slow"),
            $('.day-event[date-month="' + a + '"][date-day="' + b + '"][data-number="' + c + '"]')
              .find(".save")
              .attr("checked", !1),
            $('.day-event[date-month="' + a + '"][date-day="' + b + '"][data-number="' + c + '"]')
              .find("span")
    ...
    m(h.pDate.month, j),
      $(".btn-prev").on("click", function (a) {
        ...
        if ($(".events__calendar").attr("data-activedate") != "") {
          var activeDate = $(".events__calendar").attr("data-activedate").trim();
          $(".events__calendar td[date-year='" + activeDate.split("/")[0] + "'][date-month='" + +activeDate.split("/")[1] + "'][date-day='" + +activeDate.split("/")[2] + "']").addClass("active-date");
        }
      }),
      $(".btn-next").on("click", function (a) {
        var b = $(".month").attr("data-month");
        if (b < 2) {
      ...
      $(".save").click(function () {
        if (this.checked) {
          ...
          $(".person-list").append('<div class="day" date-month="' + b + '" date-day="' + c + '" data-number="' + d + '" style="display:none;">' + a + "</div>"),
            $('.day[date-month="' + b + '"][date-day="' + c + '"]').slideDown("fast"),
          ...
          var b = $(this).closest(".day-event").attr("date-month"),
            c = $(this).closest(".day-event").attr("date-day"),
            d = $(this).closest(".day-event").attr("data-number");
          $('.day[date-month="' + b + '"][date-day="' + c + '"][data-number="' + d + '"]').slideUp("slow"),
            setTimeout(function () {
...
const findEventsInCalendar = function () {
  ...
  $(".day-event").each(function (i, elem) {
    eventDates[i] = [$(elem).attr("date-year"), $(elem).attr("date-month"), $(elem).attr("date-day")];
  });
  $(".event-calendar td").each(function (i, elem) {
    var year = $(elem).attr("date-year"),
      month = $(elem).attr("date-month"),
      day = $(elem).attr("date-day");
    if (year != "") {
      for (var j = 0; j < eventDates.length; j++) {
        if (!$(elem).hasClass("has")) {
          if (year === eventDates[j][0] && month === eventDates[j][1] && day === eventDates[j][2]) {
            var temp = $(".day-event[date-day=" + eventDates[j][2] + "][date-year=" + eventDates[j][0] + "][date-month=" + eventDates[j][1] + "]");
            $(elem).addClass("has");
            if ($(temp).length > 1) {
              let eventContainer;
              eventContainer = $(`<div class="tooltip-container"></div>`).html($(temp).clone());
