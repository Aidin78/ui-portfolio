...
! function () {
    ...
    }, persian_Date = A, pDate = A, persian_Date.unix = persian_Date.prototype.unix, persian_Date.utc = persian_Date.prototype.utc
}(), persian_Date().format();
var calendar = {
    init: function (a) {
    ...
    startCalendar: function () {
        ...
        function s(a, b) {
            $($("tbody.event-calendar tr")).each(function (a) {
                $(this).empty()
            }), workDate = persian_Date([b, a, 1]), offset = workDate.gDate.getDay() + 1, 7 == offset && (offset = 0);
            for (var c = 0; c < offset; c++) $("tbody.event-calendar tr.1").append("<td></td>");
            for (var d = 1; d <= n(a, b); d++) index = d + offset, index < 8 ? $("tbody.event-calendar tr.1").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>") : index < 15 ? $("tbody.event-calendar tr.2").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>") : index < 22 ? $("tbody.event-calendar tr.3").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>") : index < 29 ? $("tbody.event-calendar tr.4").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>") : index < 36 ? $("tbody.event-calendar tr.5").append('<td date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>") : $("tbody.event-calendar tr.6").append('<td data-toggle="tooltip" date-month="' + a + '" date-day="' + d + '" date-year="' + b + '">' + r(d) + "</td>");
        ...
        m(h.pDate.month, j), $(".btn-prev").on("click", function (a) {
...
$(document).ready(function () {
    ...
    $('.calendar tbody td').on('click', function () {
        ...
        if (parseInt(day) <= parseInt(persianToEnglish(today[2]))) {
            var month = this.getAttribute('date-month');
            var year = this.getAttribute('date-year');
            window.open('[esprit:query:21]/' + year + '/' + month + '/' + day);
        }
    });
    var today = new Date().toLocaleDateString('fa-IR')
    today = today.split('/');
...
/*var weather__slider = document.querySelectorAll('.weather__slider');
weather__slider.forEach(function(sliderElem){
    var weekday = ['شنبه', 'یک شنبه', 'دوشنبه', 'سه شنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه'];
    var todayInPersian = weekday.findIndex(element => element == '[esprit:date:202]')
    var weather__item = sliderElem.querySelectorAll('.weather__item');
    var weathercounter = 0;
    weather__item.forEach(function (elem) {
        var elem__day = elem.querySelector('.weather__day');
        var elem__icon = elem.querySelector('.weather__icon img');
        var elem__date = elem.querySelector('.weather__date');
        elem__day.innerHTML = weekday[(weathercounter + todayInPersian) % 7];
        elem__date.innerHTML = ([query-result:day] + weathercounter) + "[query-result:shamsimonth]";
        elem.setAttribute('title',weather.daily[weathercounter].weather[0].description)
        var imageAddress = '/uploads/news/assets/img/' + weather.daily[weathercounter].weather[0].icon;
        elem__icon.setAttribute('src', imageAddress);
