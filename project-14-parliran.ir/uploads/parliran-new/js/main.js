...
const mainMenu = document.querySelector("#parliranmenu");
let menuTitle;
let moreTitle;
let menuPos;
if (siteRtl == "rtl") {
...
function initMenu() {
  let smMenu = $("#parliranmenu").clone();

  // Making Mmenu for phone
  $("#parliranmenu > ul").removeClass("sm sm-rtl sm-simple");
  initMmenu();
}

function initMmenu() {
  (function (e) {
    new Mmenu(mainMenu, { searchfield: { add: true, title: "جستجو" }, extensions: ["light", "position-front", "position-" + menuPos], navbar: { add: !0, title: menuTitle } }, { clone: 0 });
  }.apply(this, [jQuery]));
}
...
function multimediaCarousel() {
  let multimediaCarousel = $(".multimedia__tab-content .multimedia__carousel");
